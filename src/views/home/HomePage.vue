  ...
  <div class="md:p-16 p-3 bg-gray-primary">
      ...
      <div class="">
        <div class="relative w-full">
          <div class="absolute left-3 top-1/2 transform -translate-y-1/2 flex justify-start gap-2">
            ...
            <p class="w-20 text-xs text-nowrap overflow-hidden text-primary">
              San Diego, CA
            </p>
          </div>

          <input type="text" class="bg-white pl-32 w-full text-black rounded-lg border-black border px-4 py-2 md:px-2"
            placeholder="Search..." />
    ...
    <section class="md:grid gap-10 md:grid-cols-4 w-full my-5 flex flex-col-reverse">
      <section class="md:col-span-1 md:h-4/6 flex md:block flex-row justify-normal gap-4">
        ...
        <div class="p-5 my-2 h-60 border border-black w-full center text-black rounded-lg">
          <h1>Banner 1</h1>
        </div>
        <div class="p-5 my-2 h-60 border border-black w-full center text-black rounded-lg">
          <h1>Banner 2</h1>
        ...
        <div class="flex justify-around w-full flex-wrap gap-2 my-5">
          <div v-for="(item, index) in postlist" :key="index" class=" cursor-pointer">
            <router-link to="/post/12">
              <PostDetail :title="item?.title" :location="item?.location" :date="item?.date" :image="item?.image" />
            </router-link>
          </div>
        </div>
      </section>
...
export default {
  ...
  computed: {
    ...
    ActiveCategoryRoute: function () {
      ...
      if (this.activeCategory === 'Legal/Lawyers') {
        return '/services/lawyer/explore'
      } else {
        return '/'
      }
    }
  ...
  methods: {
    handleSlideRight() {
      document.getElementById("service-container").scrollBy({
        top: 0,
    ...
    handleActiveCategory(data) {
      this.activeCategory = JSON.parse(JSON.stringify(data))?.name;
    },
...
<style scoped>
.banner-bg {
  background: linear-gradient(88.8deg, #008080 34.51%, #00d5d5 130.17%);
}
